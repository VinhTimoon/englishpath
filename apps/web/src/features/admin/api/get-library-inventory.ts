...
function apiBase() {
  ...
  try {
    ...
    throw new LibraryInventoryError(0);
  }
...
function item(value: unknown): LibraryInventoryItem {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new LibraryInventoryError(500);
  ...
  if (
    ...
    typeof v.canReview !== "boolean" ||
    typeof v.canPublish !== "boolean"
  )
    throw new LibraryInventoryError(500);
  return Object.fromEntries(
    [...strings, "canReview", "canPublish"].map((key) => [key, v[key]]),
  ) as LibraryInventoryItem;
}
...
export async function getLibraryInventory(
  ...
  try {
    response = await fetch(`${apiBase()}/cms/library/inventory`, {
      signal,
      headers: {
    ...
    throw new LibraryInventoryError(0, true);
  }
  if (response.status === 404 || response.status === 501)
    throw new LibraryInventoryError(response.status, true);
