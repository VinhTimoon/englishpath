...
export type ShadowingState = { item: LibraryState["item"]; attempt: { attemptKey: string; segmentIndex: number; positionSeconds: number; status: string; selfRating: number | null; finalizedAt: string | null } | null; history: Array<{ attemptKey: string; status: string; selfRating: number | null; finalizedAt: string | null }> };
export function getShadowing(versionId: string, signal?: AbortSignal) { return requestLearnerApi<{ data: ShadowingState }>(`/library/items/${encodeURIComponent(versionId)}/shadowing`, { signal }).then((r) => r.data); }
export function saveShadowing(versionId: string, body: { segmentIndex: number; positionSeconds: number; status: "active" | "paused"; selfRating?: number }) { return requestLearnerApi<{ data: ShadowingState["attempt"] }>(`/library/items/${encodeURIComponent(versionId)}/shadowing`, { method: "PUT", body }).then((r) => r.data); }
export function finalizeShadowing(versionId: string, body: { segmentIndex: number; positionSeconds: number; selfRating: number }) { return requestLearnerApi<{ data: NonNullable<ShadowingState["attempt"]> }>(`/library/items/${encodeURIComponent(versionId)}/shadowing/finalize`, { method: "POST", body }).then((r) => r.data); }
