...
const migration = readFileSync(join(root, 'prisma/migrations/20260806210000_toeic_question_governance/migration.sql'), 'utf8');
...
describe('TOEIC question governance schema', () => {
  ...
  it('keeps answer, provenance, rights, and review evidence on the version model', () => {
    expect(schema).toMatch(/model ToeicQuestionVersion[\s\S]*correctAnswer\s+String/);
    expect(schema).toMatch(/sourceIdentity[\s\S]*sourceUrl[\s\S]*checksum[\s\S]*rightsOwner/);
    expect(schema).toMatch(/reviewEvidence[\s\S]*reviewerIdentity[\s\S]*reviewedAt/);
    expect(schema).not.toMatch(/ToeicQuestionVersion[\s\S]*public/);
  });

  it('enforces stable version and import identities with lifecycle indexes', () => {
    expect(schema).toMatch(/@@unique\(\[questionId, version\]\)/);
    expect(schema).toMatch(/@@unique\(\[sourceIdentity, checksum, sourceVersion\]\)/);
    expect(schema).toMatch(/@@unique\(\[importIdentity\]\)/);
    expect(schema).toMatch(/@@index\(\[reviewStatus, publicationState, publishedAt, validUntil\]\)/);
  ...
  it('contains only additive executable migration SQL', () => {
    ...
    expect(executable).not.toMatch(/\b(DROP|DELETE|TRUNCATE|ALTER\s+TABLE[\s\S]*\b(RENAME|DROP|ALTER))\b/i);
